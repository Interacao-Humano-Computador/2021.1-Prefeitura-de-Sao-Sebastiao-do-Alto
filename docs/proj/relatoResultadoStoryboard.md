# Avaliação Storyboard

## Introdução

<p style="text-indent: 20px; text-align: justify"> 
Depois de ter preparado o planejamento da avaliação desses storyboards, seguimos com a avaliação feitas pelos membros do grupo. Com o método escolhido, que foi por meio de entrevistas com as personas traçadas durante a etapa de perfis de usuários foi permitido ao avaliador ter contato, interpretar e analisar as concepções, opiniões, desejos, e ações do usuário relacionado com sistemas interativos, além de permitir investigar alternativas de design, problemas que os usuários enfrentam utilizar o sistema no dia a dia e como eles lidam com a tecnologia existente no sistema.
</p>

<p style="text-indent: 20px; text-align: justify"> 
Com esses dados dos perfis de usuário, conseguimos obter informações valiosas a respeito do nosso storyboard.
</p>



## Objetivo

<p style="text-indent: 20px; text-align: justify">  Com essa avaliação do storyboard, temos como objetivo validar o protótipo feito para cada uma das funcionalidade vistas nesses cenários. Demonstrando as tarefas do sistema através desse protótipo.</p>

## Metodologia

<p style="text-indent: 20px; text-align: justify"> 
O grupo realizou a escolha de utilizar como método para esta avaliação a <b>entrevista</b>, que faz parte do método de  investigação. Com base nisso e na coleta de dados levantado durante a etapa de perfil de usuário, escolhemos entrevistar as personas levantadas para o projeto, para avaliarem os storyboards feitos pelo grupo. Todas as personas entrevistadas consentiram com os termos de consentimento para a realização da entrevista. As perguntas aplicadas serão as mesma para todos os entrevistados para não haver parcialidade. Segue a baixo os entrevistados:
</p>

## Entrevistados

### Carlos Silva Alves

<p> &emsp; Carlos é uma persona do levantada para o projeto. Ele é um comerciante que gosta de se manter informado sobre as notícias de sua cidade. Ele foi convidado para avaliar os storyboards.</p>

###  Perguntas gerais do questionário

| Número | Pergunta        | Estrutura da Resposta                                                                    |
| ------ | --------------- | ---------------------------------------------------------------------------------------- |
| 1      | Qual seu nome?  | Carlos Silva                                                                             |
| 2      | Qual sua idade  | 43 anos                                                                                  |
| 3      | Qual seu gênero | [ x ] Masculino<br/>[&emsp;] Feminino<br/>[&emsp;] Não binário<br/>[&emsp;] Outro, qual? |
<center>
<figcaption>Tabela 1 - Questionário geral</figcaption>
</center>

<br>

#### Questionário do Storyboard 1 - IPTU

| Número | Pergunta                                                           | Estrutura da Resposta                                                                                     |
| ------ | ------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------- |
| 4      | Você acredita que o cenário ilustrado representa um caso real?                  | [ x ] Sim</br>[&emsp;] Não                                                                                |
| 5      | Qual a sugestão de alteração ou melhoria para o storyboard sugere? | Nenhuma sugestão. Acredito que o "desenho" ilustra bem um caso em que a função IPTU do site seria utilizada. Áte por que eu utilizo ela.|

<center>
<figcaption>Tabela 2 - Questionário Storyboard 1</figcaption>
</center>

<br>

#### Questionário do Storyboard 2 - Boletim COVID-19

| Número | Pergunta                                                                                      | Estrutura da Resposta      |
| ------ | --------------------------------------------------------------------------------------------- | -------------------------- |
| 6      | Esse segundo storyboard, você acredita que representa bem a situação de um cidadão preocupado com a situação do covid no município?                                                | [ x ] Sim</br>[&emsp;] Não |
| 7      | A cena retrata bem a funcionalidade observada na situação? | Acredito que sim, porém deveria ter retratado mais a funcionalidade em funcionamento. |

<center>
<figcaption>Tabela 2 - Questionário Storyboard 2</figcaption>
</center>

<br>

#### Questionário do Storyboard 3 - História do município

| Número | Pergunta                                                                                      | Estrutura da Resposta      |
| ------ | --------------------------------------------------------------------------------------------- | -------------------------- |
| 6      | Nesse terceiro storyboard, você acredita que representa bem a situação de uma pessoa querer conhecer a história da cidade através do site?                                               | [ x ] Sim</br>[&emsp;] Não |
| 7      | A cena retrata bem a funcionalidade observada na situação? | Achei bastante interessante, vito que eu que sou um comerciante não conhecia essa parte do site da prefeitura, é sempre bom se manter informado sobre a história da cidade para contar aos meus clientes. |

<center>
<figcaption>Tabela 3 - Questionário Storyboard 4</figcaption>
</center>

<br>

### Ana Carolina Pereira de Aguiar

<p> &emsp; Ana Carolina é outra persona que foi levantada para o projeto. Ela é uma advogada que atua no município, com um grande senso de independência. Ela foi convidado para avaliar os storyboards.</p>

###  Perguntas gerais do questionário

| Número | Pergunta        | Estrutura da Resposta                                                                    |
| ------ | --------------- | ---------------------------------------------------------------------------------------- |
| 1      | Qual seu nome?  | Ana Carolina                                                                            |
| 2      | Qual sua idade  | 28 anos                                                                                  |
| 3      | Qual seu gênero | [&emsp;] Masculino<br/>[ x ] Feminino<br/>[&emsp;] Não binário<br/>[&emsp;] Outro, qual? |
<center>
<figcaption>Tabela 4 - Questionário geral</figcaption>
</center>

<br>

#### Questionário do Storyboard 1 - IPTU

| Número | Pergunta                                                           | Estrutura da Resposta                                                                                     |
| ------ | ------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------- |
| 4      | Você acredita que o cenário ilustrado representa um caso real?                  | [ x ] Sim</br>[&emsp;] Não                                                                                |
| 5      | Qual a sugestão de alteração ou melhoria para o storyboard sugere? | Acredito que o storyboard poderia ter detalhado mais o processo de obter esse IPTU pelo site, mas olhando o contexto em geral representa bem e de maneira bem simples.|

<center>
<figcaption>Tabela 5 - Questionário Storyboard 1</figcaption>
</center>

<br>

#### Questionário do Storyboard 2 - Boletim COVID-19

| Número | Pergunta                                                                                      | Estrutura da Resposta      |
| ------ | --------------------------------------------------------------------------------------------- | -------------------------- |
| 6      | Esse segundo storyboard, você acredita que representa bem a situação de um cidadão preocupado com a situação do covid no município?                                                | [&emsp;] Sim</br>[ x ] Não |
| 7      | A cena retrata bem a funcionalidade observada na situação? | A cena não ta bem detalhada mostrando essa preocupação do cidadão com a COVID-19, ele apenas se questiona se existe tal funcionalidade, porém não cumpre com o papel definido. |



<center>
<figcaption>Tabela 6 - Questionário Storyboard 2</figcaption>
</center>

<br>

#### Questionário do Storyboard 3 - História do município

| Número | Pergunta                                                                                      | Estrutura da Resposta      |
| ------ | --------------------------------------------------------------------------------------------- | -------------------------- |
| 6      | Nesse terceiro storyboard, você acredita que representa bem a situação de uma pessoa querer conhecer a história da cidade através do site?                                               | [ x ] Sim</br>[&emsp;] Não |
| 7      | A cena retrata bem a funcionalidade observada na situação? | Esse storyboard esta muito bem representado, mostrando que quando uma pessoa de fora do município vier visitar, deseja ter um conhecimento prévio da nossa cidade, e tem onde buscar que seria o site da prefeitura. |

<center>
<figcaption>Tabela 7 - Questionário Storyboard 4</figcaption>
</center>

<br>

## Conclusão

<p style="text-indent: 20px; text-align: justify"> 
Com as informações e dados coletados nas entrevistas dessas personas, observamos que em geral como o nosso storyboard aborda o escopo do projeto. Com o Carlos Alves que gostou de todos storyboards e apenas sugeriu um detalhamento maior no storyboard - Boletim do COVID, chegando a conhecer uma nova funcionalidade do site, que seria conhecer a história do município através dele. E com a avaliação da advogada Ana Carolina que gostou dos storyboards de uma maneira geral, porém faltando um pouco de detalhe no processo da funcionalidade do storyboard - IPTU e não estando de acordo com o storyboard - Boletim COVID, precisando de uma maior atenção a ele. 
</p>

<p style="text-indent: 20px; text-align: justify"> 
Com isso concluimos as entrevistas e a avaliação dos storyboards criados pelo grupo sobre a Prefeitura de São Sebastião do Alto.
</p>

## Bibliografias

BARBOSA, S.D.J.; SILVA, B.S. Interação Humano--Computador.Editora Campus- Elsevier, 2010.


## Versionamento

| Versão | Data | Modificação | Autor |
|:--:|:--:|:--:|:--:|
| 1.0 | 10/09/2021 | Criação do documento de relato do resultado | Gabriel Avelino |
| 1.1 | 11/09/2021 | Adição das entrevistas das personas | Gabriel Avelino |
| 1.2 | 12/09/2021 | Revisão do documento | Ítalo Serra (Revisor) |